...
Files: billups/transforms.py owns Silver and Gold logic; billups/pipeline.py owns local reads, persistence and quality evidence; billups/report.py renders bounded aggregate findings; billups/download.py fetches the official inputs; tests/ verifies semantics and reruns. Dependencies are PySpark for required processing and pytest for test execution. Use the standard library for downloads, manifests and reports.

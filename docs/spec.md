...
## Reproducibility
Run into a fresh output directory. Reusing an existing run fails without deleting it. Publish a SUCCESS.json marker only after all outputs, DQ, report and previews exist. Identical inputs must yield identical business results on another run; elapsed timing may differ. Failed runs have no completion marker and may contain partial artifacts. Full Gold stays local; commit only bounded previews and the report.
